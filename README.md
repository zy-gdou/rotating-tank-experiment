...
2. $\zeta=\nabla^2\psi=(\partial_{xx}+\partial_{yy})\psi$ is the relative voriticy;
3. $k_d=1/R_d$ with the deformation radius $R_d=\sqrt{gH}/f_0$, the gravitational acceleration $g$, mean water depth $H$, andthe Coriolis parameter $f_0$;
...
7. $Q$ is the forcing in terms of $\zeta$ injection per second. 
The $\psi-\zeta$ equation has been historically employed to simplify the [Shallow Water Equations](https://en.wikipedia.org/wiki/Shallow_water_equations). In practice, an additional visocisity term $A_h \nabla^2\zeta$ is needed in the RHS in order to damp the short waves and maintain the numerical stability, where $A_h$ is the kenimatic vicosity. 
...
"genXYgrid_vect.m" defines the boundary points(the points located at the side wall of the tank), ghost points(points dropping outside of the tank with  a distance of 1 $dx$ to the neighouring boundary points, where $dx$ is the grid size), and inside points(points dropping inside the tank accounting for the major part of the total node population). This classification is useful when implementing the boundary conditions. Two boundary conditions (no-slip and free-slip) can be applied to the lateral boundaries using one of the schemes in the review paper by E, Weinan & Liu, Guoqiang 1996(Journal of Computational Physics). According to our experiment setup, the lateral boundary includes the side wall of the cylindrical tank and 3 sides of the barrier, ; the barrier is assumed to be a thin rectangle in "genXYgrid_vect.m" with one side joining the tank wall. 

'sweep_run.m' defines the parameter space we are going to survey and run the model for each combination of these parameters. The key parameters are:
